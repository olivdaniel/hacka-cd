...
(() => {
  ...
  /* ------------------------------------------------------------------ */
  const API = {
    async saveRecord(record, status) {
      let session;
      try {
      ...
      if (!session?.token || !record?.id) return;
      const response = await fetch(`${RECORDS_API_BASE}/api/records/${encodeURIComponent(record.id)}`, {
        method: "PUT",
      ...
      if (!response.ok) {
        ...
        const error = new Error(result.title || result.detail || "Não foi possível persistir a NC no backend.");
        error.status = response.status;
        throw error;
      }
    },

    /** Visão gerencial: registros do servidor (todos, para perfis de gestão; senão, os próprios). */
    async listRecords() {
      const session = JSON.parse(sessionStorage.getItem(SESSION_KEY) || "null");
      if (!session?.token) throw new Error("Sessão não autenticada.");
      const response = await fetch(`${RECORDS_API_BASE}/api/records`, { headers: { Authorization: `Bearer ${session.token}` } });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        const error = new Error(result.title || result.detail || "Não foi possível listar os registros.");
        error.status = response.status;
        throw error;
      }
      return result;
    ...
    async reviewRecord(recordId) {
      const session = JSON.parse(sessionStorage.getItem(SESSION_KEY) || "null");
      if (!session?.token) throw new Error("Sessão não autenticada.");
      const response = await fetch(`${RECORDS_API_BASE}/api/records/${encodeURIComponent(recordId)}/review`, {
        method: "POST",
        headers: { Authorization: `Bearer ${session.token}` },
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        const error = new Error(result.title || result.detail || "Não foi possível revisar a NC.");
        error.status = response.status;
        throw error;
      }
      return result;
    ...
    async getUser() {
      ...
      if (saved) {
        try {
          ...
          if (account.name && account.email) return { ...account, initials: account.name.split(/\s+/).map((part) => part[0]).join("").slice(0, 2).toUpperCase(), agentName: "Ctrl + CD" };
        } catch {
    ...
    async sendMessage(payload, signal) {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.title || "Falha ao comunicar com o agente.");
      return result;
    },
  };
  ...
  async function sendMessage() {
    ...
    textNode.innerHTML = `<div class="thinking"><span class="spinner"></span>Consultando…</div>`;

    try {
      const result = await API.sendMessage({ chatId: chat.id, message: content });
      if (result.chatId && result.chatId !== chat.id) {
        chat.id = result.chatId;
        Store.activeId = result.chatId;
      }
      chat.title = chat.title || content.slice(0, 30);
      assistant.content = result.answer || "O agente não retornou uma resposta.";
      assistant.sources = Array.isArray(result.sources) ? result.sources : [];
    } catch (error) {
  ...
  async function checkConnection() {
    ...
    try {
      ...
      if (!response.ok || result.status !== "ok") throw new Error("health failed");
      status.textContent = "Experimental Lab conectado";
  ...
  const HASH_ROUTES = { gerencial: "experiment", dashboard: "experiment", registro: "experiment", chat: "chat", assistente: "chat", hacka: "hacka" };
  ...
  async function init() {
    ...
    if (connectionStatus) {
      ...
      connectionStatus.dataset.status = "checking";
    }
    checkConnection();
